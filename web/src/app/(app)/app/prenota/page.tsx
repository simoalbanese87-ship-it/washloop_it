...
import { BookFlow, type Address, type Laundry, type Slot, type SpecialCategory } from "@/components/app/BookFlow";
...
export default async function PrenotaPage() {
  ...
  const [active, { data: addresses }, { data: laundries }, { data: slots }, { data: cats }, { data: items }] = await Promise.all([
  ...
  return (
    <div className="space-y-4">
      {!active ? (
        ...
        <BookFlow addresses={addresses!} laundries={laundries ?? []} slots={slots ?? []} categories={categories} />
      )}
    </div>
  );
}
