...
export default async function SegnalazioniAperte({
  ...
  let q = svc
    .from("order_issues")
    .select(
      "id, order_id, kind, capo, testo, photo_url, created_at, published_at, resolved_at, resolution, orders!inner(status, profiles!orders_customer_id_fkey!inner(full_name, client_code, is_test))",
    )
