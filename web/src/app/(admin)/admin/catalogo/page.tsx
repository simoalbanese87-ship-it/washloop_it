...
type Plan = { id: string; name: string; price_month_cents: number; turnaround_hours: number; pickups_per_week: number; bags_per_week: number; active: boolean; stripe_price_id: string | null };
...
export default async function CatalogoPage({ searchParams }: { searchParams: Promise<{ ok?: string; warn?: string }> }) {
  ...
  const [{ data: zones }, { data: laundries }, { data: slots }, { data: plans }, { data: couriers }] = await Promise.all([
    ...
    supabase
      ...
      .limit(600)
      .returns<Slot[]>(),
    supabase.from("plans").select("id, name, price_month_cents, turnaround_hours, pickups_per_week, bags_per_week, active, stripe_price_id").order("sort").returns<Plan[]>(),
          ...
          <div className="mt-4 space-y-4">
            {(plans ?? []).map((p) => (
              <div key={p.id} className="rounded-[14px] border border-line p-4">
                <form action={updatePlan} className="grid gap-2 sm:grid-cols-[1.4fr_1fr_1fr_1fr_1fr_auto_auto] sm:items-end">
