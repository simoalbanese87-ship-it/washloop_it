...
/** Export CSV dei contatti.
 *  Rispetta gli stessi filtri della pagina (?q=, ?zona=, ?stato=, ?fonte=,
 *  ?nuovi=): scarichi quello che vedi.
...
const HEADERS = [
  ...
  "Provenienza", "UTM source", "UTM medium", "UTM campaign",
];
...
export async function GET(req: Request) {
  ...
  const nuovi = searchParams.get("nuovi");
  const settimanaFa = Date.now() - 7 * 86_400_000;

  const svc = createServiceClient();
  const { data } = await svc
    .from("leads")
    .select("full_name, email, phone, cap, plan, covered, contact_status, source, created_at, utm, zones(name)")
  ...
  return new NextResponse(body, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="washloop-contatti-${today}.csv"`,
