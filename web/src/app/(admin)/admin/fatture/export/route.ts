...
export async function GET(req: Request) {
  ...
  const mese = searchParams.get("mese"); // YYYY-MM, opzionale
  ...
  if (mese && /^\d{4}-\d{2}$/.test(mese)) {
    ...
    const da = new Date(Date.UTC(anno, m - 1, 1)).toISOString();
    const a = new Date(Date.UTC(m === 12 ? anno + 1 : anno, m === 12 ? 0 : m, 1)).toISOString();
    q = q.gte("created_at", da).lt("created_at", a);
  }
  ...
  const nome = mese ? `washloop-incassi-${mese}.csv` : `washloop-incassi-tutti.csv`;
